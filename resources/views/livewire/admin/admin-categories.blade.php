<div class="grid gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
    <x-ui.card class="h-fit">
        ...
        @if($errors->any())<x-ui.alert variant="danger" class="mt-4">{{ $errors->first() }}</x-ui.alert>@endif
                ...
                <tbody>
                    @foreach($categories as $category)
                        <tr wire:key="category-row-{{ $category->id }}">
                            ...
                            <td><div class="ui-table-actions"><x-ui.button wire:click="edit({{ $category->id }})" variant="secondary" size="sm">Edit</x-ui.button>@if(!$category->is_protected && !$category->products_count)<x-ui.confirm-action action="delete({{ $category->id }})" title="Hapus kategori" message="Kategori {{ $category->name }} akan dihapus." confirm-label="Hapus" button-variant="danger" size="sm">Hapus</x-ui.confirm-action>@endif</div></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
