<div class="mx-auto max-w-5xl space-y-6">
    <div>
        ...
        <p class="mt-1 text-sm text-muted">Informasi ini digunakan pada header, footer, checkout, invoice, email, dan perhitungan ongkir.</p>
    </div>
    ...
    <x-ui.card>
        <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-muted">Alamat ini menjadi asal pengiriman untuk setiap perhitungan ongkir.</p>
            </div>
        ...
        <div class="mt-4">
            ...
            <textarea id="store-address" wire:model="address" rows="3" class="ui-field" placeholder="Nama jalan, nomor bangunan, dan patokan"></textarea>
            <x-input-error :messages="$errors->get('address')" />
        </div>

        <div class="mt-4 grid gap-4 md:grid-cols-2">
            <x-ui.searchable-select
                wire:model.live="selected_province_id"
                :options="$provinces"
                label="Provinsi"
            ...
            />
            <x-ui.searchable-select
                wire:model.live="selected_city_id"
                :options="$cities"
                ...
                :instance-key="'store-city-'.$selected_province_id"
            />
            <x-ui.searchable-select
                wire:model.live="selected_district_id"
                :options="$districts"
                label="Kecamatan"
                ...
                :instance-key="'store-district-'.$selected_city_id"
            />
            <x-ui.searchable-select
                wire:model.live="selected_subdistrict_id"
                :options="$subdistricts"
                label="Kelurahan/Desa"
                placeholder="Pilih kelurahan/desa"
                search-placeholder="Cari kelurahan/desa..."
                :clearable="false"
                :disabled="empty($subdistricts)"
                :instance-key="'store-subdistrict-'.$selected_district_id"
            />
            <div>
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-2">
            @foreach(['instagram', 'facebook', 'tiktok', 'youtube'] as $social)
                <div>
                    ...
                    <input id="social-{{ $social }}" wire:model="social_links.{{ $social }}" placeholder="URL {{ ucfirst($social) }}" class="ui-field">
