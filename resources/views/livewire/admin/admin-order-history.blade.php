    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <table class="w-full border-collapse text-center bg-white">
            <thead class="bg-gray-50">
                <tr>
                    <th class="py-3 px-2 border-b-2 border-gray-200">Order ID</th>
                    <th class="py-3 px-2 border-b-2 border-gray-200">Customer</th>
                    <th class="py-3 px-2 border-b-2 border-gray-200">Total Amount</th>
                    <th class="py-3 px-2 border-b-2 border-gray-200">Status</th>
                    <th class="py-3 px-2 border-b-2 border-gray-200">Order Date</th>
            ...
            <tbody">
                @forelse($orders as $order)
                    <tr>
                        ...
                        <td class="py-3 px-2 border-b border-gray-200">{{ $order->user->name }}</td>
                        <td class="py-3 px-2 border-b border-gray-200">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</td>
                        <td class="py-3 px-2 border-b border-gray-200">{{ ucfirst($order->status) }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="py-10 text-center text-gray-500">No orders found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
