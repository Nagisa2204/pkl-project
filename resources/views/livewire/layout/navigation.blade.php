...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 sticky top-0 z-50">
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-gray-200 bg-white text-gray-700 font-semibold text-sm hover:bg-gray-50 transition-all">
                <a href="{{ route('dashboard') }}" wire:navigate
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" 
                        class="w-4 h-4 {{ request()->routeIs('home') ? '!text-white' : 'text-gray-700' }}">
                ...
                @auth
                    <a href="{{ route('orders.history') }}" wire:navigate
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" 
                            class="w-4 h-4 {{ request()->routeIs('orders.*') ? '!text-white' : 'text-gray-700' }}">
            ...
            <div class="hidden sm:flex sm:items-center sm:gap-3">
                ...
                <a href="{{ route('cart') }}" wire:navigate 
                   class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-gray-200 bg-white text-gray-700 font-semibold text-sm hover:bg-gray-50 transition-all">
                ...
                @guest
                    <a href="{{ route('login') }}" wire:navigate class="text-sm font-semibold text-gray-700 hover:text-black px-3 py-2">
                        Masuk
                    </a>
                    <a href="{{ route('register') }}" wire:navigate class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-black text-white font-semibold text-sm hover:bg-gray-800 transition-all">
                        Buat Akun
                    </a>
                @endguest
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        @else
            <div class="pt-2 pb-3 space-y-1 border-t border-gray-200 px-4">
                <a href="{{ route('login') }}" wire:navigate class="block w-full py-2 text-center text-sm font-semibold text-gray-700">Masuk</a>
                <a href="{{ route('register') }}" wire:navigate class="block w-full py-2 text-center text-sm font-semibold text-white bg-black rounded-lg">Buat Akun</a>
            </div>
        @endauth
    </div>
</nav>
