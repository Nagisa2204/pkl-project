@props(['product', 'showAction' => true, 'showCategory' => true])
...
<article {{ $attributes->class('ui-product-card') }}>
    <a href="{{ $detailUrl }}" class="relative block overflow-hidden bg-subtle" wire:navigate>
        @if ($image)
            <img src="{{ Storage::url($image->image_path) }}" alt="{{ $image->alt_text ?: $product->name }}"
                class="h-52 w-full object-cover transition duration-300 hover:scale-[1.02]" loading="lazy">
        @else
        ...
        <div class="absolute left-3 top-3 flex flex-wrap gap-2">
            ...
            @elseif($compareAtPrice > $price)
                <x-ui.badge variant="success">Diskon
                    {{ (int) round((1 - $price / $compareAtPrice) * 100) }}%</x-ui.badge>
            @endif
        </div>
    </a>
    ...
    <div class="flex flex-1 flex-col p-4">
        ...
        @endif
        <a href="{{ $detailUrl }}" class="mt-1 line-clamp-2 font-bold leading-5 text-content hover:text-primary"
        ...
        @if ($showAction)
            <div class="mt-auto pt-4">
                @if ($hasChoices)
                    <x-ui.button :href="$detailUrl" variant="secondary" class="w-full" wire:navigate>Pilih
                        varian</x-ui.button>
                @else
                    <x-ui.button wire:click="addToCart({{ $variant?->id ?? 0 }})" wire:loading.attr="disabled"
                        wire:target="addToCart({{ $variant?->id ?? 0 }})" :variant="$isPurchasable ? 'secondary' : 'outline'" :disabled="!$isPurchasable"
                        class="w-full">
